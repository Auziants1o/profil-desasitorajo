...
<div class="page-hero">
    <div class="container">
        ...
        <div class="breadcrumb"><a href="{{ route('beranda') }}">Beranda</a><i class="fas fa-chevron-right"></i><span>Struktur Organisasi</span></div>
    </div>
</div>
            ...
            <div style="min-width: 1000px; display: inline-block;">
                ...
                <div style="display: flex; justify-content: space-between; padding-top: 30px; gap: 15px; position: relative;">
                    @foreach([
                        ...
                        <div style="position: absolute; top: 0; left: calc({{ ($idx * 16.666) + 8.333 }}%); width: 2px; height: 30px; background: var(--primary);"></div>
                        ...
                        <div style="margin-top: 30px; position: relative; width: 145px; display: flex; flex-direction: column;">
                            ...
                            <div class="org-node" style="background: var(--bg-white); border: 1px solid var(--border); padding: 15px 10px; border-radius: 8px; box-shadow: var(--shadow);">
                                <div style="font-weight: 700; color: var(--primary-dark); font-size: 11px; margin-bottom: 8px; line-height: 1.4;">{{ $st[0] }}</div>
                                <div style="font-weight: 700; font-size: 12px;">{{ $st[1] }}</div>
                            </div>
                            ...
                            @if($st[2])
                                ...
                                <div class="org-node" style="background: #fef9c3; border: 1px solid #ca8a04; padding: 10px 8px; border-radius: 6px; box-shadow: var(--shadow);">
                                    <div style="font-weight: 700; font-size: 10px; color: #a16207; margin-bottom: 3px;">{{ $st[3] }}</div>
                                    <div style="font-weight: 700; font-size: 11px; color: #422006;">{{ $st[4] }}</div>
                                </div>
                                ...
                                <div style="display: flex; justify-content: space-between; gap: 6px;">
                                    <div class="org-node" style="background: #dbeafe; border: 1px solid #2563eb; padding: 8px 4px; border-radius: 6px; flex: 1;">
                                        <div style="font-weight: 700; font-size: 9px; color: #1e40af; margin-bottom: 3px;">{{ $st[5] }}</div>
                                        <div style="font-weight: 700; font-size: 10px; color: #172554;">{{ $st[6] }}</div>
                                    </div>
                                    <div class="org-node" style="background: #dbeafe; border: 1px solid #2563eb; padding: 8px 4px; border-radius: 6px; flex: 1;">
                                        <div style="font-weight: 700; font-size: 9px; color: #1e40af; margin-bottom: 3px;">{{ $st[7] }}</div>
                                        <div style="font-weight: 700; font-size: 10px; color: #172554;">{{ $st[8] }}</div>
                                    </div>
                                </div>
                            @endif
                        </div>
